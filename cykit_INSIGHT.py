...
class EEG_insight(object):
    ...
    def convert_v2(self, value_1, value_2): #Converte os dados adquiridos para o formato de tensão
    ...
    def data_list(self):
        ...
        while 1:
            ...
            AF3 = float(eeg_data[EEG_name["AF3"]])
            T7 = float(eeg_data[EEG_name["T7"]])
            PZ = float(eeg_data[EEG_name["Pz"]])
            T8 = float(eeg_data[EEG_name["T8"]])
            AF4 = float(eeg_data[EEG_name["AF4"]])
            ...
            self.data.append(datetime.now()) #Lista com instantes de aquisição de cada dado - Exemplo: '2021-10-13 10:53:48.813976'
    ...
    def gen_file(self, filename): #Gera arquivo de gravação e salva os dados adquiridos
        ...
        with open(filename, "w") as f:
            ...
            for c in range(len(self.canal_AF3)):
                ...
                linha = self.canal_AF3[c] + self.delimiter + self.canal_T7[c] + self.delimiter + self.canal_Pz[c] + self.delimiter + self.canal_T8[c] + self.delimiter + self.canal_AF4[c] + self.delimiter + str(self.data[c]) + "\n"
                f.write(linha)
